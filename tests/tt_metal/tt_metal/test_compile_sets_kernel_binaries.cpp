                ...
                ths.emplace_back([&] {
                    for (int j = 0; j < num_compiles; j++) {
                        ...
                        ll_api::memory const& brisc_binary = llrt::get_risc_binary(
                            brisc_hex_path, 0, 0, 0, ll_api::memory::Packing::CONTIGUOUS, ll_api::memory::Relocate::XIP);
                        TT_FATAL(
                        ...
                        ll_api::memory const& ncrisc_binary =
                            llrt::get_risc_binary(ncrisc_hex_path, 0, 1, 0, ll_api::memory::Packing::CONTIGUOUS, relo_type);
                        TT_FATAL(
