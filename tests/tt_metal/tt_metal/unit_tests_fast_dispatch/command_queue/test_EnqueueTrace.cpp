...
TEST_F(SingleDeviceTraceFixture, WriteReadSanity) {
    ...
    for (uint32_t loop_idx=0; loop_idx<num_loops; loop_idx++) {
        ...
        // auto buffer = Buffer::create(this->device_, 128, 128, BufferType::DRAM);
        ...
        auto buffer = CreateBuffer(dram_config);
        ...
        // For dev/debug go ahead and print the results. Was seeing weird things with first element during replay.
