...
std::vector<Tensor> ExecuteUnaryBackwardLogit::invoke(const Tensor& grad, const Tensor& input, const std::optional<MemoryConfig>& output_mem_config) {
    ...
    Tensor status = ttnn::logical_and(
        ...
        output_mem_config);
    grad_result = where(
        ttnn::eq(status, 1.0f, std::nullopt, output_mem_config), grad_result, std::nanf(""));
    grad_result = where(
        ttnn::logical_or(
...
std::vector<Tensor> ExecuteUnaryBackwardRelu6::invoke(const Tensor& grad, const Tensor& input, const std::optional<MemoryConfig>& output_mem_config) {
    std::vector<Tensor> grad_tensor;
    Tensor grad_result =
        where(ttnn::le(input, 0.0f, std::nullopt, output_mem_config), 0.0f, 6.0f, output_mem_config);
    grad_result = where(
        ttnn::logical_and(
        ...
        output_mem_config);
    grad_result =
        where(ttnn::ge(input, 6.0f, std::nullopt, output_mem_config), 0.0f, grad_result, output_mem_config);
...
std::vector<std::optional<Tensor>> ExecuteUnaryBackwardSilu::invoke(uint8_t queue_id, const Tensor& grad, const Tensor& input, const std::optional<MemoryConfig>& output_mem_config, std::optional<Tensor> input_grad) {
    ...
    Tensor add_sub = ttnn::add(queue_id,
        ttnn::multiply(queue_id, ttnn::rsub(sigmoid_res, 1.0f, output_mem_config),
            input,
            std::nullopt,
            output_mem_config),
        1.0f,
...
std::vector<Tensor> ExecuteUnaryBackwardCosh::invoke(const Tensor& grad, const Tensor& input, const std::optional<MemoryConfig>& output_mem_config) {
    ...
    Tensor t_neg_inf =
        ttnn::multiply(ttnn::sign(grad, output_mem_config), -std::numeric_limits<float>::infinity(), std::nullopt, output_mem_config);
    Tensor grad_a = where(
        ttnn::gt(input, 88.50f, std::nullopt, output_mem_config), t_inf,
        where(
...
std::vector<Tensor> ExecuteUnaryBackwardDivNoNan::invoke(
    ...
    Tensor result = where(
        ttnn::eq(val, 0, std::nullopt, output_mem_config), 0.0, ttnn::multiply(grad, 1 / scalar, std::nullopt, output_mem_config), output_mem_config);
    grad_tensor.emplace_back(result);
    return grad_tensor;
}
...
std::vector<Tensor> ExecuteUnaryBackwardRecip::invoke(
    ...
    grad_tensor.emplace_back(where(
        ...
        where(
            ttnn::eqz(grad, output_mem_config),
            t_nan,
            ttnn::multiply(ttnn::neg(ttnn::sign(grad, output_mem_config), output_mem_config), t_inf, std::nullopt, output_mem_config),
            output_mem_config),
        ttnn::multiply(ttnn::neg(grad, output_mem_config),
...
std::vector<ComplexTensor> ExecuteUnaryBackwardAbs::invoke(
    ...
    Tensor grad_inp_r = where(ttnn::eqz(result, output_mem_config), 0.0, ttnn::multiply(grad, ttnn::multiply(input.real(), ttnn::reciprocal(result, output_mem_config), std::nullopt, output_mem_config),std::nullopt, output_mem_config), output_mem_config );
    Tensor grad_inp_i = where(ttnn::eqz(result, output_mem_config), 0.0, ttnn::multiply(grad, ttnn::multiply(input.imag(), ttnn::reciprocal(result, output_mem_config), std::nullopt, output_mem_config),std::nullopt, output_mem_config), output_mem_config );
...
std::vector<Tensor> ExecuteUnaryBackwardProd::invoke(
    ...
    if (dim == 3 || dim == -1) {
        ...
        if (reciprocal_input.get_legacy_shape()[1] % 32 != 0) {
            ttnn::SmallVector<std::pair<uint32_t, uint32_t>> padding = {{0, 0},
    ...
    if (reciprocal_input.get_legacy_shape()[0] % 32 != 0) {
        ttnn::SmallVector<std::pair<uint32_t, uint32_t>> padding = {{0, (32 - (reciprocal_input.get_legacy_shape()[0] % 32))},
                      {0, 0},
                      {0, 0},
                      {0, 0}};
