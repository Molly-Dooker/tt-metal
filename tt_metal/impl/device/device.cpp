...
std::vector<CoreCoord> Device::get_optimal_dram_bank_to_worker_core_assignment() {
    if (not this->optimal_dram_bank_to_worker_core_assignment_.size()) {
        ...
        for (int i = 0; i < physical_worker_cores.size(); ++i) {
            for (int j = 0; j < all_worker_cores_logical.size(); ++j) {
                ...
                if (physical_worker_cores[i] == core) {
                    this->optimal_dram_bank_to_worker_core_assignment_.push_back(all_worker_cores_logical[j]);
                }
            }
        }
    }
    return this->optimal_dram_bank_to_worker_core_assignment_;
}
