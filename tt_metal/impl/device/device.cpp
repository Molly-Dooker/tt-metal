...
uint32_t Device::num_eth_worker_cores(uint32_t sub_device_index) const {
    return this->num_eth_worker_cores_[sub_device_index];
}

uint32_t Device::num_worker_cores(uint32_t sub_device_index) const {
    return this->num_worker_cores_[sub_device_index];
}
...
void Device::initialize_cluster() {
    ...
    // TODO: This will be set when setting up sub devices
    this->num_worker_cores_[0] = this->compute_with_storage_grid_size().x * this->compute_with_storage_grid_size().y;
    this->num_eth_worker_cores_[0] = this->get_active_ethernet_cores(true).size();
        ...
        switch(dwv) {
            ...
            case DispatchWorkerType::DISPATCH:
            {
                ...
                if (num_dispatchers == 1 || num_dispatchers == 2) {
                    ...
                    for (auto&[core, settings] : device_worker_variants[DispatchWorkerType::DISPATCH]) {
                        ...
                        compile_args[21] = 1; // unused: dispatch_d only. max_num_worker_sems is used for array sizing, set to 1
            ...
            case DispatchWorkerType::DISPATCH_D:
            {
                ...
                for (auto&[core, dispatch_d_settings] : device_worker_variants[DispatchWorkerType::DISPATCH_D]) {
                    ...
                    compile_args[22] = dispatch_d_settings.compute_core_mcast_noc_coords;
            ...
            case DispatchWorkerType::DISPATCH_S:
            {
                if (this->dispatch_s_enabled()) {
                    ...
                    for (auto&[core, dispatch_s_settings] : device_worker_variants[DispatchWorkerType::DISPATCH_S]) {
                        ...
                        compile_args[11] = dispatch_message_base_addr;
                        compile_args[12] = max_dispatch_message_entries;
                        dispatch_s_idx++;
                    }
                }
                break;
...
void Device::reset_num_sub_devices(uint32_t num_sub_devices) {
    TT_FATAL((num_sub_devices >=1 && num_sub_devices <= dispatch_constants::DISPATCH_MESSAGE_ENTRIES), "Illegal number of sub devices specified");
    ...
    this->sysmem_manager_->reset_config_buffer_mgr(num_sub_devices);
    ...
    num_sub_devices_ = num_sub_devices;
}

uint32_t Device::num_sub_devices() const {
    return num_sub_devices_;
}
