...
// FIXME - Why seeing multiple definitions of functions here unless inline?
...
inline void PrintHostDataType(const HostDataType& data) {
    std::visit([](const auto& value) {
        ...
        if constexpr (std::is_same_v<T, const std::shared_ptr<std::vector<uint8_t>>>) {
            std::cout << "HostDataType contains: std::shared_ptr<std::vector<uint8_t>>" << std::endl;
        } else if constexpr (std::is_same_v<T, const std::shared_ptr<std::vector<uint16_t>>>) {
            std::cout << "HostDataType contains: std::shared_ptr<std::vector<uint16_t>>" << std::endl;
        } else if constexpr (std::is_same_v<T, const std::shared_ptr<std::vector<int32_t>>>) {
            std::cout << "HostDataType contains: std::shared_ptr<std::vector<int32_t>>" << std::endl;
        } else if constexpr (std::is_same_v<T, const std::shared_ptr<std::vector<uint32_t>>>) {
            std::cout << "HostDataType contains: std::shared_ptr<std::vector<uint32_t>>" << std::endl;
        } else if constexpr (std::is_same_v<T, const std::shared_ptr<std::vector<float>>>) {
            std::cout << "HostDataType contains: std::shared_ptr<std::vector<float>>" << std::endl;
        } else if constexpr (std::is_same_v<T, const std::shared_ptr<std::vector<bfloat16>>>) {
            std::cout << "HostDataType contains: std::shared_ptr<std::vector<bfloat16>>" << std::endl;
        } else if constexpr (std::is_same_v<T, const void*>) {
            std::cout << "HostDataType contains: const void*" << std::endl;
        } else {
            std::cout << "HostDataType contains: Unknown type" << std::endl;
        }
    }, data);
}
...
// Steps to tracing/replaying a new host API function.
// 1. Define flatbuffer schema table for the command (commands.fbs update)
// 2. Define capture helper function with same signature as host API being captured to add host API "command" to the command vector
// 3. Call the capture helper function from the host API function via macro TRACE_FUNCTION_CALL(func, args...)
// 4. Define the replay helper function called "execute()" that processes command (unpack from flatbuffer, call host API) and add fwd decl in hpp
...
inline void captureCreateBuffer(std::shared_ptr<Buffer> buffer, const InterleavedBufferConfig &config) {
    ...
    uint32_t buffer_global_id = ctx.getBufferGlobalId(buffer.get());
...
inline void captureDeallocateBuffer(Buffer *buffer) {
    ...
    captureCommand(tt::target::CommandType::DeallocateBufferCommand,
        tt::target::CreateDeallocateBufferCommand(ctx.getBuilder(), ctx.getBufferGlobalId(buffer)).Union());
}
...
inline void captureEnqueueWriteBuffer(CommandQueue& cq, std::variant<std::reference_wrapper<Buffer>, std::shared_ptr<Buffer>> buffer, HostDataType src, bool blocking) {
    ...
    uint32_t buffer_global_id = ctx.getBufferGlobalId(buffer_ptr);
