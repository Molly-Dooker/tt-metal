...
static_assert(mux_fan_in > 0 && mux_fan_in <= MAX_SWITCH_FAN_IN, "mux fan-in 0 or higher than MAX_SWITCH_FAN_IN");
static_assert(MAX_SWITCH_FAN_IN == 4, "MAX_SWITCH_FAN_IN must be 4 for the initialization below to work");
...
// Input queue local write index address
constexpr uint32_t mux_in_local_wptr_addr[MAX_SWITCH_FAN_IN] = {
    get_compile_time_arg_val(25),
    get_compile_time_arg_val(26),
    get_compile_time_arg_val(27),
    get_compile_time_arg_val(28)};

// As the Mux reads the inputs from each queue it updates their read sent/cleared values
constexpr uint32_t mux_in_remote_rptr_sent_addr[MAX_SWITCH_FAN_IN] = {
    get_compile_time_arg_val(29),
    get_compile_time_arg_val(30),
    get_compile_time_arg_val(31),
    get_compile_time_arg_val(32)};

constexpr uint32_t mux_in_remote_rptr_cleared_addr[MAX_SWITCH_FAN_IN] = {
    get_compile_time_arg_val(33),
    get_compile_time_arg_val(34),
    get_compile_time_arg_val(35),
    get_compile_time_arg_val(36)};

constexpr uint32_t mux_out_local_rptr_sent_addr = get_compile_time_arg_val(37);
constexpr uint32_t mux_out_local_rptr_cleared_addr = get_compile_time_arg_val(38);
// Remote output queue where the Mux will be writing it's outputs
constexpr uint32_t mux_out_remote_wptr_addr = get_compile_time_arg_val(39);
...
void kernel_main() {
    ...
    write_buffer_to_l1(test_results, PQ_TEST_MISC_INDEX + 1, 0xaa000000 | mux_fan_in);
    ...
    for (uint32_t i = 0; i < mux_fan_in; i++) {
        input_queues[i].init(
            ...
            mux_in_local_wptr_addr[i],
            mux_in_remote_rptr_sent_addr[i],
            mux_in_remote_rptr_cleared_addr[i],
            input_packetize[i],
    ...
    output_queue.init(
        ...
        input_queues,
        mux_fan_in,
        mux_out_local_rptr_sent_addr,
        mux_out_local_rptr_cleared_addr,
        mux_out_remote_wptr_addr,
        output_depacketize,
        output_depacketize_log_page_size,
        output_depacketize_downstream_sem,
        output_depacketize_local_sem,
    ...
    while (!dest_finished && !timeout) {
        ...
        if (input_queues[curr_input].get_curr_packet_valid()) {
            ...
            uint32_t words_sent = output_queue.forward_data_from_input(
                curr_input, full_packet_sent, input_queues[curr_input].get_end_of_cmd());
