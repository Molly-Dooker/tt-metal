...
static_assert(
    demux_fan_out > 0 && demux_fan_out <= MAX_SWITCH_FAN_OUT, "demux fan-out 0 or higher than MAX_SWITCH_FAN_OUT");
static_assert(MAX_SWITCH_FAN_OUT == 4, "MAX_SWITCH_FAN_OUT must be 4 for the initialization below to work");
...
static_assert(
    (demux_fan_out < 2) || is_power_of_2(remote_tx_queue_size_words[1]),
    "remote_tx_queue_size_words must be a power of 2");
static_assert(
    (demux_fan_out < 3) || is_power_of_2(remote_tx_queue_size_words[2]),
    "remote_tx_queue_size_words must be a power of 2");
static_assert(
    (demux_fan_out < 4) || is_power_of_2(remote_tx_queue_size_words[3]),
    "remote_tx_queue_size_words must be a power of 2");
...
constexpr DispatchRemoteNetworkType remote_rx_network_type =
    static_cast<DispatchRemoteNetworkType>(get_compile_time_arg_val(19));

static_assert(
    MAX_DEST_ENDPOINTS <= 32 && MAX_SWITCH_FAN_OUT <= 4,
...
constexpr uint8_t dest_output_queue_id_map[MAX_DEST_ENDPOINTS] = {
    (dest_endpoint_output_map_lo >> 0) & 0x3,  (dest_endpoint_output_map_lo >> 2) & 0x3,
    (dest_endpoint_output_map_lo >> 4) & 0x3,  (dest_endpoint_output_map_lo >> 6) & 0x3,
    (dest_endpoint_output_map_lo >> 8) & 0x3,  (dest_endpoint_output_map_lo >> 10) & 0x3,
    (dest_endpoint_output_map_lo >> 12) & 0x3, (dest_endpoint_output_map_lo >> 14) & 0x3,
    (dest_endpoint_output_map_lo >> 16) & 0x3, (dest_endpoint_output_map_lo >> 18) & 0x3,
    (dest_endpoint_output_map_lo >> 20) & 0x3, (dest_endpoint_output_map_lo >> 22) & 0x3,
    (dest_endpoint_output_map_lo >> 24) & 0x3, (dest_endpoint_output_map_lo >> 26) & 0x3,
    (dest_endpoint_output_map_lo >> 28) & 0x3, (dest_endpoint_output_map_lo >> 30) & 0x3,
    (dest_endpoint_output_map_hi >> 0) & 0x3,  (dest_endpoint_output_map_hi >> 2) & 0x3,
    (dest_endpoint_output_map_hi >> 4) & 0x3,  (dest_endpoint_output_map_hi >> 6) & 0x3,
    (dest_endpoint_output_map_hi >> 8) & 0x3,  (dest_endpoint_output_map_hi >> 10) & 0x3,
    (dest_endpoint_output_map_hi >> 12) & 0x3, (dest_endpoint_output_map_hi >> 14) & 0x3,
    (dest_endpoint_output_map_hi >> 16) & 0x3, (dest_endpoint_output_map_hi >> 18) & 0x3,
    (dest_endpoint_output_map_hi >> 20) & 0x3, (dest_endpoint_output_map_hi >> 22) & 0x3,
    (dest_endpoint_output_map_hi >> 24) & 0x3, (dest_endpoint_output_map_hi >> 26) & 0x3,
    (dest_endpoint_output_map_hi >> 28) & 0x3, (dest_endpoint_output_map_hi >> 30) & 0x3};
...
// Local write index for the Demux input
constexpr uint32_t demux_in_local_wptr_addr = get_compile_time_arg_val(30);
// As the Demux reads data from the input it will update these sent/cleared values on the remote
constexpr uint32_t demux_in_remote_rptr_sent_addr = get_compile_time_arg_val(31);
constexpr uint32_t demux_in_remote_rptr_cleared_addr = get_compile_time_arg_val(32);

// Demux local rtpr sent and cleared addresses for each fan out
constexpr uint32_t demux_out_local_rptr_sent_addr[MAX_SWITCH_FAN_OUT] = {
    get_compile_time_arg_val(33),
    get_compile_time_arg_val(34),
    get_compile_time_arg_val(35),
    get_compile_time_arg_val(36)};

constexpr uint32_t demux_out_local_rptr_cleared_addr[MAX_SWITCH_FAN_OUT] = {
    get_compile_time_arg_val(37),
    get_compile_time_arg_val(38),
    get_compile_time_arg_val(39),
    get_compile_time_arg_val(40)};

// As the Demux forwards data to the remote and will update the write pointer at the remote
constexpr uint32_t demux_out_remote_wptr_addr[MAX_SWITCH_FAN_OUT] = {
...
void kernel_main() {
    ...
    write_buffer_to_l1(test_results, PQ_TEST_MISC_INDEX + 1, 0xbb000000 | demux_fan_out);
    write_buffer_to_l1(test_results, PQ_TEST_MISC_INDEX + 2, dest_endpoint_output_map_hi);
    write_buffer_to_l1(test_results, PQ_TEST_MISC_INDEX + 3, dest_endpoint_output_map_lo);
    write_buffer_to_l1(test_results, PQ_TEST_MISC_INDEX + 4, endpoint_id_start_index);

    input_queue.init(
        0,
        rx_queue_start_addr_words,
        rx_queue_size_words,
        remote_rx_x,
        remote_rx_y,
        remote_rx_queue_id,
        remote_rx_network_type,
        demux_in_local_wptr_addr,
        demux_in_remote_rptr_sent_addr,
        demux_in_remote_rptr_cleared_addr);

    for (uint32_t i = 0; i < demux_fan_out; i++) {
        output_queues[i].init(
            i + 1,
            remote_tx_queue_start_addr_words[i],
            remote_tx_queue_size_words[i],
            remote_tx_x[i],
            remote_tx_y[i],
            remote_tx_queue_id[i],
            remote_tx_network_type[i],
            &input_queue,
            1,
            demux_out_local_rptr_sent_addr[i],
            demux_out_local_rptr_cleared_addr[i],
            demux_out_remote_wptr_addr[i],
            output_depacketize[i],
            output_depacketize_log_page_size[i],
            output_depacketize_local_sem[i],
            output_depacketize_downstream_sem[i],
    ...
    while (!all_outputs_finished && !timeout) {
        ...
        if (input_queue.get_curr_packet_valid()) {
            ...
            uint32_t words_sent = output_queues[output_queue_id].forward_data_from_input(
                0, full_packet_sent, input_queue.get_end_of_cmd());
