...
static_assert(router_lanes > 0 && router_lanes <= MAX_SWITCH_FAN_OUT, "demux fan-out 0 or higher than MAX_SWITCH_FAN_OUT");
...
void kernel_main() {
    ...
    for (uint32_t i = 0; i < router_lanes; i++) {
        input_queues[i].init(
            i,
            rx_queue_start_addr_words + i * rx_queue_size_words,
            rx_queue_size_words,
            remote_rx_x[i],
            remote_rx_y[i],
            remote_rx_queue_id[i],
            remote_rx_network_type[i],
            vc_packet_router_in_local_wptr_addr[i],
            vc_packet_router_in_remote_rptr_sent_addr[i],
            vc_packet_router_in_remote_rptr_cleared_addr[i],
            input_packetize[i],
            input_packetize_log_page_size[i],
            input_packetize_local_sem[i],
    ...
    for (uint32_t i = 0; i < router_lanes; i++) {
        output_queues[i].init(
            ...
            &input_queues[i],
            1,
            vc_packet_router_out_local_rptr_sent_addr[i],
            vc_packet_router_out_local_rptr_cleared_addr[i],
            vc_packet_router_out_remote_wptr_addr[i],
    ...
    while (!all_outputs_finished && !timeout) {
        ...
        if (input_queues[curr_input].get_curr_packet_valid()) {
            ...
            uint32_t words_sent = output_queues[curr_input].forward_data_from_input(
                0, full_packet_sent, input_queues[curr_input].get_end_of_cmd());
    ...
    if (!timeout) {
        write_buffer_to_l1(kernel_status, PQ_TEST_MISC_INDEX, 0xff000003);
        for (uint32_t i = 0; i < router_lanes; i++) {
