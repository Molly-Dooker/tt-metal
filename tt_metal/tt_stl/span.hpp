...
template <class T, std::size_t Extent>
Span<const std::byte, Extent == dynamic_extent ? dynamic_extent : sizeof(T) * Extent> as_bytes(
    Span<T, Extent> other) noexcept {
    return decltype(stl::as_bytes(other)){reinterpret_cast<const std::byte*>(other.data()), other.size_bytes()};
}

template <class T, std::size_t Extent>
Span<
    std::enable_if_t<not std::is_const_v<T>, std::byte>,
    Extent == dynamic_extent ? dynamic_extent : sizeof(T) * Extent>
as_writable_bytes(Span<T, Extent> other) noexcept {
    return decltype(stl::as_writable_bytes(other)){reinterpret_cast<std::byte*>(other.data()), other.size_bytes()};
}
