...
inline std::vector<T> untilize_nchw(const BufferType<T>& in, const PhysicalSize& shape, std::optional<PhysicalSize> tile_shape = std::nullopt) {
    ZoneScoped;
    std::vector<T> result;
    if(in.size() == 0) {
        return result;
...
template <typename T, template <typename...> typename BufferType>
inline std::vector<T> tilize_nchw(const BufferType<T>& in_rowmajor, const PhysicalSize& shape, std::optional<PhysicalSize> tile_shape = std::nullopt) {
    ZoneScoped;
    ...
    switch (inL) {
        ...
        case tests::utils::TensorLayoutType::LIN_ROW_MAJOR:
            if (outL == tests::utils::TensorLayoutType::TILED_SWIZZLED) {
                ...
                return convert_to_tile_layout<T>(swiz32, tile_shape, face_shape, transpose_within_face, transpose_of_faces);
            } else
                TT_ASSERT(false && "Unsupported conversion.");
        break;
        case tests::utils::TensorLayoutType::TILED_NFACES:
            if (outL == tests::utils::TensorLayoutType::TILED_SWIZZLED) {
                ...
                auto swiz32 = convert_to_flat_layout<T>(inp, tile_shape, face_shape, transpose_within_face, transpose_of_faces);
...
inline std::vector<T> convert_layout(
    ...
    return convert_layout<T, BufferType>(inp, PhysicalSize{H, W},  inL, outL, tile_shape, face_shape, transpose_within_face, transpose_of_faces);
}
