...
def test_perf_squeezenet(mesh_device, batch_size, expected_inference_time, expected_compile_time):
    ...
    filename = "models/demos/wormhole/squeezenet/demo/dog_image.jpeg"
    ...
    with torch.no_grad():
        ...
        for _ in range(2):
            ...
            tt_input = ttnn.from_torch(torch.permute(input_batch, (0, 2, 3, 1)), mesh_mapper=inputs_mesh_mapper)
            ...
            tt_out = ttnn.to_torch(tt_out, mesh_composer=output_mesh_composer)
            end = time.time()
...
def test_perf_device_bare_metal(batch_size, reset_seeds):
    ...
    command = f"pytest tests/ttnn/integration_tests/squeezenet/test_squeezenet_wh.py"
